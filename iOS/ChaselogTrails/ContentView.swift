...
struct ContentView: View {
    ...
    @State private var editingEntry: ChaseEntry?
        ...
        NavigationStack {
            ZStack {
                ...
                if store.entries.isEmpty {
                    ContentUnavailableView("No Chase Log Entries Yet", systemImage: "square.and.pencil", description: Text("Tap + to add your first entry."))
                        ...
                        ForEach(store.entries) { entry in
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    Text(entry.eventType.isEmpty ? "Untitled" : entry.eventType)
            ...
            .navigationTitle("Chaselog Trails")
            .toolbar {
...
struct EntryEditorView: View {
    ...
    @State private var draft: ChaseEntry
    @State private var draftDate: Date = Date()

    init(entry: ChaseEntry?) {
        self.entry = entry
        _draft = State(initialValue: entry ?? ChaseEntry())
        ...
        NavigationStack {
            ZStack {
                ...
                TextField("Event Type", text: $draft.eventType)
                    .accessibilityIdentifier("field_eventType")
